...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
    ...
    const [intermediateUpdate, setIntermediateUpdate] = useState<string>("");
    const { sendMessageToAgent } = useAgent(instance, writeFileSync);
   ...
   const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setInput("");
    setIsLoading(true);
    setIntermediateUpdate("");

    try {
      const contextualMessage = getChatModePrompt(chatMode, input.trim());

      const finalResponse = await sendMessageToAgent(
        contextualMessage,
        messages.slice(-10),
        chatMode,
        model,
        (update) => setIntermediateUpdate(update)
      );
